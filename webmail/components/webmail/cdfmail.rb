...
class CDF::Attachment
  ...
  def filename=(arg) @filename = arg end
  ...
  def file
    File.open(self.temp_filename) { |fp| fp.read }
  end
    
  def file=(data)
    return if data.size == 0
    @content_type = data.content_type
    @filename = data.original_filename.scan(/[^\\]*$/).first
    @temp_filename = "#{@mail.temp_location}/#{@filename}"
    check_store_path
    data.rewind
  ...
  def body(data, fb)
    ...
    @filename = filename.nil? ? (mime_encoded?(data.subject) ? mime_decode(data.subject) : data.subject) : filename
    @temp_filename = "#{@mail.temp_location}/#{@filename}"
    check_store_path
